...
class Clean_Selector(Selector):
    def __init__(self):
        super(Clean_Selector, self).__init__()
        
    def select(self, wav_list, max_dur, criterion="random"):
        wav_list = super(Clean_Selector, self).select(wav_list, max_dur, criterion)
        utt_list = [wav_path.split("/")[-1] for wav_path in wav_list]
        return wav_list, utt_list

class Noise_Selector(Selector):
    def __init__(self):
        super(Noise_Selector, self).__init__()
    def select(self, wav_list, max_dur, criterion="random"):
        wav_list = super(Noise_Selector, self).select(wav_list, max_dur, criterion)
        return wav_list