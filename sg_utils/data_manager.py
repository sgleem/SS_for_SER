...
class DataManager:
    ...
    def get_wav_path(self, split_type=None, *args, **kwargs):
        wav_root=self.env_dict["audio_path"]
        if split_type == None:
            wav_list = glob.glob(os.path.join(wav_root, "*.wav"))
        else:
            utt_list = self.get_utt_list(split_type)
    ...
    def __load_msp_cat_label_dict__(self):
        label_path = self.env_dict["label_path"]
    ...
    def __load_msp_dim_label_dict__(self):
        label_path = self.env_dict["label_path"]
        self.msp_label_dict=dict()
    ...
    def get_msp_labels(self, utt_list, lab_type=None):
        if lab_type == "categorical":
            self.__load_msp_cat_label_dict__()
        elif lab_type == "dimensional":
            self.__load_msp_dim_label_dict__()
